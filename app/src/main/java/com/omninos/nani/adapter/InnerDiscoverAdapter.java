 ...
 */
public class InnerDiscoverAdapter extends RecyclerView.Adapter<InnerDiscoverAdapter.MyViewHolder> {

    Context context;
    private List<DiscoverModelClass.PostList> postLists;
    Select select;
    ...
    public interface Select {
        ...
        void Subscrib(int position);
    }

    public InnerDiscoverAdapter(Context context, List<DiscoverModelClass.PostList> postLists, Select select) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, final int position) {
        if (!postLists.get(position).getNaniImage().isEmpty()) {
            Glide.with(context).load(postLists.get(position).getNaniImage()).into(holder.dp_vertical_feed_item);
        }
        holder.name__vertical_feed_item.setText(postLists.get(position).getNaniNane());
        holder.address.setText(postLists.get(position).getNaniAddress());
        holder.rating_bar_vertical_feed_item.setRating(Float.parseFloat(postLists.get(position).getRating()));
        Glide.with(context).load(postLists.get(position).getPostImage()).into(holder.food_image_vertical_feed_item);
        holder.food_image_vertical_feed_item.setScaleType(ImageView.ScaleType.CENTER_CROP);
        holder.donuts_text_vertical_feed_item.setText(postLists.get(position).getName());
        holder.nice_good_vertical_feed_item.setText(postLists.get(position).getDescription());
        holder.price.setText("R " + postLists.get(position).getPrice());
